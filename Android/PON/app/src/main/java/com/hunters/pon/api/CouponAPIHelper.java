...
//        String token = Constants.HEADER_AUTHORIZATION.replace("%s", CommonUtils.getToken(context));
    ...
    public void removeFavouriteCoupon(Context context, String couponId , final Handler handler)
    {
        if(NetworkUtils.isNetworkAvailable(context)) {
            showProgressDialog(context);
            Retrofit retrofit = new Retrofit.Builder()
            ...
            Call<ResponseCommon> response = service.removeFavouriteCoupon(token, couponId);
