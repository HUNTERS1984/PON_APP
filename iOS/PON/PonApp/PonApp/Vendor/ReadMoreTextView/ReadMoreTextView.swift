...
@objc protocol ReadMoreTextViewllerDelegate: class {
    @objc optional func didPressReadMore(sender: ReadMoreTextView)
}

@IBDesignable
class ReadMoreTextView: UITextView {
    
    weak var handler: ReadMoreTextViewllerDelegate? = nil
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if needsTrim() && pointInTrimTextRange(location!) {
            self.handler?.didPressReadMore!(sender: self)
        }
    }
